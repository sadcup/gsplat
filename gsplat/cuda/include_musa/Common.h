...
// Some Macros.
//
#define CHECK_CUDA(x) TORCH_CHECK(x.is_privateuseone(), #x " must be a CUDA tensor")
...
#define DEVICE_GUARD(_ten)                                                     \
    const at::musa::OptionalMUSAGuard device_guard(device_of(_ten));
...
#define CUB_WRAPPER(func, ...)                                                 \
    do {                                                                       \
        ...
        auto &caching_allocator = *::c10::musa::MUSACachingAllocator::get();   \
